...
class Trie {
 ...
 public:
  ...
  template <class T>
  auto PutFromNode(std::unique_ptr<TrieNode> new_node, std::string_view &key, size_t key_index,
  ...
  auto RemoveFromNode(std::unique_ptr<TrieNode> new_node, std::string_view &key, size_t key_index) const
